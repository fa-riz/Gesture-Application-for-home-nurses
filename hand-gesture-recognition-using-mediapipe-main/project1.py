...
def draw_info_text(image, brect, handedness, hand_sign_text,
    ...
    engine.say(text)
    engine.runAndWait()
    #twilio recovery code 89oco0UH2YGq8avlhPPfpDCLzObvGltdyae2EkP4
    #create table users(username varchar(10),password varchar(10));
    #create table gesture (command varchar(20),tim DATETIME);
    """
...
WHERE TIMESTAMPDIFF( SECOND, @time, tim ) >= @interval
   ...
   """
    ...
    conn = mysql.connect(host="localhost", user="root", password="Fariz@2002", database="ges")
    mycursor = conn.cursor()
    mycursor.execute("INSERT INTO gesture VALUES('" + hand_sign_text + "','" + formatted_date + "')")
